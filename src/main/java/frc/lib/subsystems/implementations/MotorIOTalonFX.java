...
public class MotorIOTalonFX implements MotorIO {
  ...
  public void setPositionSetpoint(Angle position, double ff) {
    if (config.outputMode == ClosedLoopOutputType.TorqueCurrentFOC) {
      ...
      talon.setControl(positionVoltageControl.withPosition(position).withFeedForward(ff).withEnableFOC(true));
    }
  }

  @Override
  public void setMagicalPositionSetpoint(Angle position, AngularVelocity velocity, AngularAcceleration acceleration,
      int slot, double ff) {
    if (config.outputMode == ClosedLoopOutputType.TorqueCurrentFOC) {
      talon.setControl(magicalPositionTorqueCurrentFOC.withPosition(position).withVelocity(velocity)
          .withAcceleration(acceleration).withFeedForward(ff));
    } else {
      talon.setControl(magicalPositionVoltageControl.withPosition(position).withVelocity(velocity)
          .withAcceleration(acceleration).withFeedForward(ff).withEnableFOC(true));
    }
  }
  ...
  @Override
  public void setMagicalVelcoitySetpoint(AngularVelocity velocity, AngularAcceleration acceleration, int slot) {
    if (config.outputMode == ClosedLoopOutputType.TorqueCurrentFOC) {
      talon.setControl(magicalVelocityTorqueCurrentFOC.withVelocity(velocity).withAcceleration(acceleration));
    } else {
      talon.setControl(
          magicalVelocityVoltgaeControl.withVelocity(velocity).withAcceleration(acceleration).withEnableFOC(true));
    }
  }
