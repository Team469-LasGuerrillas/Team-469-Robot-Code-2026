...
public class FieldZoning {
  ...
  public static Translation2d dynamicPassLocation() {
    ...
    double turretCurrentPoseY = Turret.getInstance().getTurretPoseFieldSpace().getY() - Constants.Field.MID_FIELD.in(Meters);
    boolean isRed = Station.isRed();
    double targetY;
    ...
    if (fromTop) {
      targetY = Constants.Field.MAX_FIELD_Y.in(Meters) - 1.469 - 0.8 * Constants.Field.MID_FIELD.in(Meters) * (turretCurrentPoseY / Constants.Field.MID_FIELD.in(Meters));
    } else {
      targetY = 1.469 - 0.8 * Constants.Field.MID_FIELD.in(Meters) * (turretCurrentPoseY / Constants.Field.MID_FIELD.in(Meters));
    }

    if (isRed) {
      return new Translation2d(Constants.Field.MAX_FIELD_X.minus(Constants.Field.PASS_WALL_DISTANCE).in(Meters), targetY);
    } else {
  ...
  public static boolean retractHood() {
    ...
    double xDeltaBlue = Math.abs(turretCurrentPose.getX() - Constants.Field.BLUE_TRENCH_SCORING.in(Meters));
    double xDeltaRed = Math.abs(turretCurrentPose.getX() - Constants.Field.RED_TRENCH_SCORING.in(Meters));

    // Blue
    boolean blueFromRight = turretSpeedsFieldSpace.vxMetersPerSecond < 0
    ...
    boolean blueFromLeft = turretSpeedsFieldSpace.vxMetersPerSecond > 0
        && turretCurrentPose.getX() < Constants.Field.BLUE_TRENCH_SCORING.in(Meters)
        && turretCurrentPose.getX() < Constants.Field.FIELD_MIDDLE.in(Meters)
        && turretCurrentPose.getX() > 0;
    ...
    boolean redFromRight = turretSpeedsFieldSpace.vxMetersPerSecond < 0
    ...
    boolean redFromLeft = turretSpeedsFieldSpace.vxMetersPerSecond > 0
        && turretCurrentPose.getX() < Constants.Field.RED_TRENCH_SCORING.in(Meters)
        && turretCurrentPose.getX() > Constants.Field.FIELD_MIDDLE.in(Meters)
        && turretCurrentPose.getX() < Constants.Field.MAX_FIELD_X.in(Meters);
    ...
    if (blueFromLeft || blueFromRight) {
      updatedZoneBlue += Constants.Field.DECAPITATION_SPEED_FACTOR
          * Math.abs(turretSpeedsFieldSpace.vxMetersPerSecond);
    }

    if (redFromLeft || redFromRight) {
      updatedZoneRed += Constants.Field.DECAPITATION_SPEED_FACTOR
          * Math.abs(turretSpeedsFieldSpace.vxMetersPerSecond);
    }
